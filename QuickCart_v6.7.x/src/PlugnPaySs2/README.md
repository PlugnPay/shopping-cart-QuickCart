...
3. Edit `plugins/plugnpay_ss2/config.php`:
   ...
   - Set `plugnpay_ss2_response_hash` to the outbound Response Verification Hash from PlugnPay Security Administration, and select its `md5` or `sha256` algorithm. Contact PlugnPay Support if the feature is not enabled.
...
- Order email is delayed until PlugnPay returns a validated success response.
...
- The return checks PlugnPay's outbound response signature, gateway account, amount, currency, order ID, and a one-time token.
...
- Do not enable the module without PlugnPay's outbound Response Verification Hash. Unsigned browser returns are not accepted as authorization results.
